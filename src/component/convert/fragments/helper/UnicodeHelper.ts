...
function isUnicode(unicodeArray: string[]): boolean {
  return unicodeArray.every((item) => !isNaN(Number(`0x${item}`)) && Number(`0x${item}`) <= 0x10ffff);
}
...
export function stringToUTF8(data: string, removeFlag: boolean = false): string {
  const inputData = data.trim();
  if (isEmpty(inputData)) {
    return '';
  }
...
export function uTF8ToString(data: string) {
  ...
  if (inputValue.includes('\\x')) {
    ...
    const decArr = getHexToDecArray(inputValue);
    const bufferArr = new Uint8Array(decArr);
    commonString = decoder.decode(bufferArr);
  }
  return commonString;
...
export function stringToUTF16(data: string, removeFlag: boolean = false): string {
  const inputData = data.trim();
  if (isEmpty(inputData)) {
    return '';
  }
  ...
  for (let i = 0; i < inputData.length; i++) {
    const code = inputData.codePointAt(i);
    if (code > 0xffff) {
      const utf16ArrString = JSON.stringify(String.fromCodePoint(code).split(''));
      utf16String += utf16ArrString.substring(2, utf16ArrString.length - 2).replace(/","/g, '');
      i++;
    } else {
  ...
  utf16String = removeFlag ? lastStringArr.join('') : `\\u${lastStringArr.join('\\u')}`;
  return utf16String;
}
 ...
 */
function convertValidHexConnectInvalid(data: string, symbol: string, codeLength: number, removeFlag: boolean): string {
  let commonString = '';
...
export function uTF16ToString(data: string, removeFlag: boolean = false) {
  ...
  let commonString = '';
  commonString = convertValidHexConnectInvalid(inputValue, '\\u', 4, removeFlag);
  return commonString;
}

export function stringToUTF32(data: string, removeFlag: boolean = false): string {
  const inputData = data.trim();
  let utf32String = '';
  for (let i = 0; i < inputData.length; i++) {
    const code = inputData.codePointAt(i);
...
export function decNumberToHTMLNameCode(data: string): string {
  ...
  let htmlNameString = '';
  const decArr = inputData.split(/\s+/) ?? [];
  decArr.forEach((item) => {
