...
export default function TimeStamp() {
  ...
  function onTimestampCopyHandler() {
    ...
    if (_window.clipboardData && _window.clipboardData.setData) {
      if (nowTimeStampType === 's') {
        return _window.clipboardData.setData('Text', Math.floor(currentTime / 1000));
      } else {
        return _window.clipboardData.setData('Text', currentTime);
      }
    } else if (navigator.clipboard?.writeText) {
      if (nowTimeStampType === 's') {
        return navigator.clipboard.writeText(Math.floor(currentTime / 1000).toString());
      } else {
  ...
  function onTimeInputChangeHandler(timeinput: React.ChangeEvent<HTMLInputElement>): void {
    setTimeInput(timeinput.target.value);
  }

  function onTimeClickHandler() {
    if (timeInput.length === 13) {
      setTimeOutput(new Date(Number(timeInput)).toLocaleString('chinese', { hour12: false }));
    } else if (timeInput.length === 10) {
      setTimeOutput(new Date(Number(timeInput) * 1000).toLocaleString('chinese', { hour12: false }));
    }
  }

  function onTimestampInputChangeHandler(timestampinput: React.ChangeEvent<HTMLInputElement>): void {
  ...
  function onTimeSelectChangeHandler(e) {
    setTimeType(e);
  }

  function onNowTimeStampSelectChangeHandler(e) {
  ...
  return (
    <div className="timestamp">
      <div className="text-stamp">
        <span className="text-stamp-title">(现在的时间戳是: </span>
        <span className="text-stamp-value">{nowTimeStampType === 's' ? Math.floor(currentTime / 1000) : currentTime}</span>
        ...
        <Button
          ...
          className="copy-button-timestamp"
          onClick={onTimestampCopyHandler}
        >
          复制
        </Button>
        <Select className="now-time-stamp" options={TYPELIST} defaultValue={nowTimeStampType} onChange={onNowTimeStampSelectChangeHandler}></Select>
      </div>
      <div className="timestamp-content">
        <div className="timestamp-content-position">
          <div className="div-flex">
            <div>
              <div className="div-flex">
                <div className="text-width">时间戳：</div>
                <div>
                  <Input type="text" className="timestamp-input" style={{ width: 250 }} value={timeInput} onChange={onTimeInputChangeHandler} />
                </div>
              </div>
              ...
              <div className="div-flex">
                <div className="text-width">北京时间：</div>
            ...
            <div>
              <div className="div-flex">
                <div className="beijingtime-text">北京时间：</div>
                <div>
                  <Input
                    type="text"
              ...
              <div className="timestamp-button-lineheight">
                <Select className="type-select" options={TYPELIST} defaultValue={timeType} onChange={onTimeSelectChangeHandler}></Select>
                <Button
                  style={{
                    color: '#FFFFFF',
                    paddingLeft: 10,
