...
export default function JSONFn() {
  ...
  function onbuttonValueClickHandler(rowElement: ILine) {
    ...
    const partnerElement = treeRef.current.find((t) => t.partner == rowElement.id);
  ...
  function onInputChangeHandler(inputvalue: React.ChangeEvent<HTMLTextAreaElement>) {
    setFontcolor('');
  ...
  function updateViewport() {
    ...
    setLinenumber(treeRef.current.filter((t) => t.showing).map((element, index) => <JSONLine key={element.id} index={index} {...element} />));
  }
  ...
  return (
    <div className="Json-div">
      ...
      <div className="JSON-input-div">
        <TextArea rows={20} value={input} onChange={onInputChangeHandler} style={{ color: fontcolor, borderRadius: 10, minHeight: 442 }} />
      </div>
      ...
      <jsonContext.Provider value={{ onToggleClick: onbuttonValueClickHandler }}>
        <div
          id="outPutContainer"
        ...
        >
          ...
          <CopyTwoTone className="button_copy" onClick={onJSONCopyClickHandler} id="TextButtonShow" />
