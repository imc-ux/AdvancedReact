...
export default function MD5() {
  ...
  function onEnctryptionChangeHandler(entryption: React.ChangeEvent<HTMLTextAreaElement>) {
    setEntryption(entryption.target.value);
  }

  function on16UpperEntryptionClickHandler() {
    setResultEntryption(Crypto.MD5(entryption).toString().toUpperCase().substring(8, 24));
  }

  function on16LowerEntryptionClickHandler() {
    setResultEntryption(Crypto.MD5(entryption).toString().toLowerCase().substring(8, 24));
  }
  ...
  return (
    <div>
      ...
      <div className="MD5-textarea" onMouseOver={onCopyButtonShowHandler} onMouseOut={onCopyButtonHiddenHandler}>
        <Row gutter={0}>
          <Col span={30}>
            <TextArea rows={10} style={{ width: 720, borderRadius: 10 }} value={resultEntryption} placeholder="这里是加密结果" readOnly />
          </Col>
        </Row>
        <div>
          <CopyTwoTone className="MD5-copy-button" onClick={oncopyToClipboardClickHandler} id="CopyButtonShow" />
        </div>
