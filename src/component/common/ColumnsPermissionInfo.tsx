...
function RendererButtonGroups(props: RendererButtonGroupsProps) {
  ...
  const [switchStatus, setSwitchStatus] = useState(permissionMsg.active === 'Y');
  ...
  return (
    <div className={space}>
      ...
      <Popconfirm title="Sure to save?" onConfirm={onBtnSaveEventHandler} onCancel={cancelMsg}>
        <Button type="primary" icon={<CheckSquareOutlined />} style={{ display: rendererDisplay }} className="button-space"></Button>
      </Popconfirm>
      <Popconfirm title="Sure to delete?" onConfirm={onBtnDelEventHandler} onCancel={cancelMsg}>
        <Button type="primary" icon={<DeleteOutlined />} style={{ display: rendererDisplay }} className="button-space"></Button>
      </Popconfirm>
      {/* </Space> */}
    </div>
  );
