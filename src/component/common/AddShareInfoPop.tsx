...
import { getCategoryList, addMyShare, myShareDetail, modifyMyShare } from '@/remote/shareAction';
...
export default function AddShareInfo(props) {
  ...
  async function initPopCategoryList(category) {
    try {
    ...
    <>
      <Draggable handle="#tttt">
        <Modal
          ...
          footer={
            modify ? (
              <Button type="primary" icon={<SaveOutlined />} onClick={onModifyBtnClickHander}>
                修改
              </Button>
            ) : (
              <Button type="primary" icon={<SaveOutlined />} onClick={onSaveBtnClickHander}>
                保存
              </Button>
            )
          }
        >
          <Radio.Group size="small" value={type} buttonStyle="solid" onChange={onRadioChangeHandler}>
          ...
          <div className="margin-t15">
            <label className="margin-r10">标题</label>
          ...
          <div className="margin-t10">
            <label>分享内容</label>
            <TextArea value={content} onChange={onTextAreaChangeHandler} showCount allowClear rows={6}></TextArea>
          </div>
          <div className="margin-t10">
            <label>分享链接</label>
