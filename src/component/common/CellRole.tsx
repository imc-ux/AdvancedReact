...
export default function EditableCell(props: EditableCellProps) {
  ...
  if (editable && record?.code !== 'manager') {
    childNode = editing ? (
      <Form.Item name={dataIndex} style={{ height: 5, lineHeight: 5, marginLeft: 20, marginRight: 20 }}>
        <Form>
          <Input
      ...
      <div className="editable-cell-value-wrap" style={{ height: 22 }} onClick={toggleEdit}>
        {children}
      </div>
    );
  }
  return (
    <td {...restProps} style={{ height: 73, padding: 0, margin: 0, textAlign: 'center' }}>
      {childNode}
    </td>
  );
}
