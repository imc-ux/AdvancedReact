...
import { Button, Table, message, Select, Input, Pagination, Spin, Popconfirm } from 'antd';
...
import { DownloadOutlined, ToTopOutlined, PlusOutlined, DeleteOutlined } from '@ant-design/icons';
...
const xlsxDocHeaderName = ['题目', '分类', '分享链接', '作者', '修改时间', '热度'];
...
export default function Share() {
    ...
    try {
      ...
      arr.data?.forEach((ele: GridDataType) => {
        if (!ele.shareLink || ele.shareLink === 'null') {
          ...
          ele.shareLinkShow = ele.shareLink.substring(ele.shareLink.lastIndexOf('/') + 1);
        } else if (ele.shareLink.indexOf('?') > -1) {
          ele.shareLinkShow = ele.shareLink.substring(0, ele.shareLink.lastIndexOf('?'));
        } else {
  ...
  function onSearchBtnClickHandler() {
    ...
    searchInfo.type = leftSortDataProvider[leftSortDataProvider.findIndex((ele) => ele.value === sort)].type;
  ...
  function DownloadDataHandler(allValue: any) {
    ...
    const worksheet = XLSX.utils.json_to_sheet(allValue, { header: xlsxDocHeaderName });
    const tableWidth = [];
  ...
  function onLeftCategoryChangeHandler(_: string, optionValue: SelectValueType) {
  ...
  return (
    <div className="fullWidth page-wrap">
      <div>
        <div className="margin-b10 fontSize16">技术分享</div>
        <div className="div-flex search-wrap">
          ...
          <div className="div-flex flex-1">
            ...
            <Input allowClear className="flex-3" value={title} onChange={onTitleChangeHandler}></Input>
          </div>
          <div className="col-gap"></div>
          <div className="div-flex flex-1">
            <label className="search-title2">内容</label>
            <Input allowClear className="flex-3" value={content} onChange={onContentChangeHandler}></Input>
          </div>
        </div>
      </div>
      <div>
        <div className="div-flex" style={{ height: '60px' }}>
          <div className="div-flex v-middle h-left flex-2">
            {showAddBtn && (
              <Button className="margin-r5" icon={<PlusOutlined />} onClick={onAddBtnClickHandler}>
                新增
              </Button>
            )}
            {showDelBtn && (
              <Popconfirm
                ...
                cancelText="取消"
              >
                <Button className="margin-r5" icon={<DeleteOutlined />} onClick={onDelBtnClickHandler}>
                  删除
                </Button>
              </Popconfirm>
            )}
            <Button className="margin-r5" icon={<DownloadOutlined />} onClick={onDownloadClickHandler}>
              下载
            </Button>
            {showTopBtn && (
              <Button className="margin-r5" icon={<ToTopOutlined />} onClick={onTotopClickHandler}>
                置顶
              </Button>
            )}
            <Select
          ...
          <div className="div-flex v-middle h-right flex-2">
            <Button className="margin-r5" onClick={onSearchBtnClickHandler}>
              检索
            </Button>
            <Button onClick={onResetBtnClickHander}>重置</Button>
          </div>
        </div>
        ...
        <div>
          ...
          <Table
            ...
            scroll={{ y: 600 }}
          ></Table>
          {modal && <AddShareInfo modalHandler={onModalCloseBtnHandler} oneRow={oneRow} isButton={isButton} />}
          {total > 0 && (
            <Pagination
