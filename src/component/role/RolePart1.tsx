...
export default function RolePart1(props: any) {
  ...
  async function onMenuDropHandler(ev: any) {
    ...
    if (role?.active) {
      const className = ev.target?.parentElement?.parentElement?.getAttribute('class');
  ...
  async function onRoleClearDropHandler(value: any) {
    ...
    if (window.confirm('Are you sure delete?')) {
      ...
      arr[index].children.splice(
        arr[index].children.findIndex((item: any) => String(item.id) === String(value.id)),
        1
      );
  ...
  function onRoleDragStartHandler(ev: React.DragEvent<HTMLParagraphElement>) {
    ...
    const dragData = { code: target.dataset.code, id: target.dataset.id, name: target.dataset.name };
    ev.dataTransfer.setData('text/plain', JSON.stringify(dragData));
    elementRef.current = document.body.appendChild(target.cloneNode(true) as HTMLElement);
    elementRef.current.setAttribute('style', 'width:200px;background-color:green;');
  ...
  return (
    <div {...otherProps} onDragOver={onDragOverHandler} onDrop={onMenuDropHandler} onDragEnd={onRoleDragEndHandler}>
      <Collapse activeKey={activeKey} bordered={false} accordion className="collapse" onChange={onChangeHandler}>
        {data?.map((e: any) => {
          return (
            <Panel id={e.userId} header={e.userName} key={e.userId} className={`col-${e.userId}`}>
              <GetUserRole
                user={e}
