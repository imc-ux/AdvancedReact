...
export default function PermissionInfoRow(prop: PermissionInfoRowProps) {
  ...
  useEffect(() => {
    if (rowInfo && prop?.children?.[0]?.props?.record?.id && prop.children[0].props.record.id === rowInfo.id) {
      setDraggable(true);
    }
  }, [rowInfo]);
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener('permissionCellEditor', onCellEditorHandler);
  }, []);

  function onCellEditorHandler(e) {
  ...
  function onRowDragStartHandler(ev: React.DragEvent<HTMLTableRowElement>) {
    ...
    if (
      ...
      const dragRow = { dragEle: prop.children?.[0].props.record, drag: 'rowData' };
      if (dragRow) {
        var img = new Image();
  ...
  return (
    <Form form={form} component={false}>
      <EditableContext.Provider value={form}>
        <tr {...props} draggable={draggable} onDragStart={onRowDragStartHandler}></tr>
