...
export default function PermissionInfo(props: PermissionInfoProps) {
  ...
  function onBtnAddRowHandler() {
    ...
    const newRow: PermissionInfo = { id: -1, code: '', name: '', description: '', active: 'Y' };
    setData([newRow, ...data]);
  }

  function onSwitchEventHandler(switchValue: CustomEvent) {
    const dataIndex = data.findIndex((item) => item.id === switchValue.detail.id);
  ...
  async function onDataBinDropHandler(ev: React.DragEvent<HTMLParagraphElement>) {
  ...
  return (
    <div className="per-table-box Info-style role-right-container">
      <div className="flex-end" style={{ margin: '0 0 10px 0', display: addBtnDispay }}>
        <Button type="primary" onClick={onBtnAddRowHandler} style={{ width: 145 }}>
          Add a Permisson
        </Button>
      </div>
      <Table
      ...
      ></Table>
      <div className="data-bin-box" onDragOver={onDataBinDragOverHandler} onDrop={onDataBinDropHandler} style={{ display: onDataBinState }}>
